...
def _compute_mdp_point(Sigma: np.ndarray, mu: np.ndarray) -> dict:
    ...
    w* ∝ Σ^{-1} σ, then rescale to sum to 1 (soft-clipped to nonnegative).
    """
...
def generate(cfg: RunConfig, seed: int, gamma: float) -> None:
    ...
    (OUTDIR / "mvo_frontier.csv").write_text(df_mvo.to_csv(index=True))
    ...
    (OUTDIR / "dro_frontier.csv").write_text(df_dro.to_csv(index=True))
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "mvo_frontier.png", dpi=150)
    plt.close(fig)

    # MVO: weights along frontier (stacked)
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "mvo_weights.png", dpi=150)
    plt.close(fig)

    # MVO: risk contributions at midpoint
    mid = len(df_mvo) // 2
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "mvo_risk.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "corr.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "dro_frontier.png", dpi=150)
    plt.close(fig)
    ...
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(OUTDIR / "mdp_point.png", dpi=150)
    plt.close(fig)
